...
export default class ResultItem extends React.Component {
  ...
  favoriteThis(pickedId){
    axios.put(/favorite/+this.props.userID, {_id: pickedId});
    this.setState({
      favorited: true
    });
  }
  ...
  UnfavoriteThis(pickedId){
    axios.put(/unfavorite/+this.props.userID, {_id: pickedId});
    this.setState({
      favorited: false
    });
  }
   ...
   blockThis(pickedId){
    axios.put(/block/+this.props.userID, {_id: pickedId});
    this.setState({
      blocked: true
    });
  }
      ...
      <div className="col-xs-12 col-sm-6 col-md-3 col-lg-2 col-xl-2 resContainer  purpleText" style={{display: this.state.blocked ? 'none':'initial'}}>
        <div className="resultItemBox">
          ...
          <img src={this.props.photo || "./images/default.png" }/>
        	<h5>{this.state.favorited && <span style={{color:"#ffb414"}} className="glyphicon glyphicon-star" aria-hidden="true"></span> }{this.props.name}</h5>
        	<p>@{this.props.username}</p>
          <div className="row">
            <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 text-right">
              <button onClick={ ()=>{ this.blockThis(this.props.id) } } type="button" className="btn block"><span style={{color:'lightpink'}} className="glyphicon glyphicon-ban-circle" aria-hidden="true"></span> Block</button>
            </div>
            <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 text-left">
              {!this.state.favorited && <button onClick={ ()=>{ this.favoriteThis(this.props.id) } } type="button" className="btn fave"><span style={{color:'hotpink'}} className="glyphicon glyphicon-plus" aria-hidden="true"></span> Fave</button>}
              {this.state.favorited && <button onClick={ ()=>{ this.UnfavoriteThis(this.props.id) } } type="button" className="btn fave2"><span style={{color:'hotpink'}} className="glyphicon glyphicon-minus" aria-hidden="true"></span>Fave</button>}
            </div>
          </div>
          <hr/>
